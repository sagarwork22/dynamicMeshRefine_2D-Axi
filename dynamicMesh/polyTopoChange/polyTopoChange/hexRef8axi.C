...
namespace Foam
{
    defineTypeNameAndDebug(hexRef8axi, 0);
...
)
{
    ...
    forAll(elems, i)
    {
        ...
        if (newI >= len)
        {
            FatalErrorIn("hexRef8axi::reorder(..)") << abort(FatalError);
        }
...
void Foam::hexRef8axi::getFaceInfo
(
    const label faceI,
    label& patchID,
    label& zoneID,
...
Foam::label Foam::hexRef8axi::addFace
(
...
// for the face)
Foam::label Foam::hexRef8axi::addInternalFace
(
    polyTopoChange& meshMod,
...
Foam::scalar Foam::hexRef8axi::getLevel0EdgeLength() const
{
    if (cellLevel_.size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef8axi::getLevel0EdgeLength() const"
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::getLevel0EdgeLength() :"
    ...
    if (level0Size == -1)
    {
        FatalErrorIn("hexRef8axi::getLevel0EdgeLength()")
...
Foam::label Foam::hexRef8axi::getAnchorCell
(
...
) const
{
    if (cellAnchorPoints[cellI].size())
    {
        ...
        FatalErrorIn("hexRef8axi::getAnchorCell(..)")
...
void Foam::hexRef8axi::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label faceI,
...
Foam::label Foam::hexRef8axi::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef8axi::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef8axi::countAnchors
(
    const labelList& f,
...
void Foam::hexRef8axi::dumpCell(const label cellI) const
{
    OFstream str(mesh_.time().path()/"cell_" + Foam::name(cellI) + ".obj");
    Pout<< "hexRef8axi : Dumping cell as obj to " << str.name() << endl;
...
Foam::label Foam::hexRef8axi::findLevel
(
    const label faceI,
    const face& f,
    const label startFp,
...
) const
{
    ...
    forAll(f, i)
    {
        ...
        if (pointLevel_[pointI] < wantedLevel)
        {
            ...
            FatalErrorIn("hexRef8axi::findLevel(..)")
                << "face:" << f
...
Foam::label Foam::hexRef8axi::getAnchorLevel(const label faceI) const
{
    const face& f = mesh_.faces()[faceI];
...
void Foam::hexRef8axi::checkInternalOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
void Foam::hexRef8axi::checkBoundaryOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
// splitPoint.
void Foam::hexRef8axi::insertEdgeSplit
(
...
Foam::label Foam::hexRef8axi::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
) const
{
   ...
   const face& f1 = mesh_.faces()[faceI];
   label anchorPointJ = f1[anchorPointI];
    bool changed = false;
    ...
    {
        midPointToAnchors.insert(edgeMidPointI, edge(anchorPointJ, -1));
    }
    else
    {
        edge& e = edgeMidFnd();
    // Pout << "edge1" << anchorPointI << endl;
   // Pout << "edgemid" << edgeMidPointI << endl;
        if (anchorPointJ != e[0])
        {
            if (e[1] == -1)
            {
  ...
  // Pout << midPointToAnchors << endl; 
    ...
    {
        edge& e = faceMidFnd();
 //    Pout << "face e" << faceMidPointI << endl;
    ...
    if (changed && haveTwoAnchors)
    {
       ...
       forAll(cFaces, i)
         ...
         //     const label& patchID = mesh_.boundaryMesh().whichPatch(faceJ);
             if (cellMidPoint[faceJ] != faceMidPointI
                && cellMidPoint[faceJ] >= 0
                && cellMidPoint[faceJ] != 123456789 )//isA<wedgePolyPatch>(mesh_.boundaryMesh()[patchID]) )
               {
                  face1  = faceJ;
//                const face& f = mesh_.faces()[faceJ];
      ...
      //   Pout << "anchors " << cellAnchorPoints[cellI][4] << endl;
       //  Pout << "second " << anchors[1] << endl;       
        label index = findIndex(cellAnchorPoints[cellI], anchorPointJ);
       // label index1 = findIndex(cellAnchorPoints[cellI], anchors[1]);
       ...
       // if (findIndex(cellAnchorPoints[cellI], anchors[1]) == 4)
        // {
         // index1 = 8;
         //}
        label point1 = cellAnchorPoints[cellI][8 - index];
       // label point2 = cellAnchorPoints[cellI][8 - index1];
        //label edgeJ = meshTools::findEdge(mesh_, point1, point2);
        label edgeMidPointJ = -1; //edgeMidPoint[edgeJ];
       
  //       Pout << "edgemid " << point1 << "d " << cellAnchorPoints[cellI] << endl;
         //Pout << " edgeJ " << edgeJ << endl;
    //     Pout << "edgeMid " << edgeMidPointJ << endl;
 ...
 //       Pout << "anchorPointI " << anchorPointI << endl;
 //      Pout << f.fcIndex(anchorPointI) << endl;
 //      Pout << f1 << endl;
 //      Pout << f << endl;
...
//         Pout << "ry " << anch << endl;
 ...
 //        Pout << "edgemid12 " << edgeMidPointJ << endl;
 ...
 //         Pout << "edgemid1234 " << edgeMidPointJ << endl;
          if (edgeMidPointI!=edgeMidPointJ)
           {
            newFaceVerts.append(edgeMidPointJ);
...
//        Pout << "storemidface" << newFace << endl;
//        vector  coordAnchor = mesh_.points()[anchorPointI];
//        vector  otherCoordAnchor = mesh_.points()[anchors.otherVertex(anchorPointI)];
        ...
        (
            ...
            cellI,
            faceI,
            anchors.otherVertex(anchorPointJ)
        );
        ...
        if (/*(coordAnchor[1] - otherCoordAnchor[1]) < 0 || (coordAnchor[0] - otherCoordAnchor[0]) < 0 )*/anchorCell0 < anchorCell1)
        {
            own = anchorCell0;
            nei = anchorCell1;

            ownPt = mesh_.points()[anchorPointJ];
            neiPt = mesh_.points()[anchors.otherVertex(anchorPointJ)];
        ...
        else
        {
            ...
            ownPt = mesh_.points()[anchors.otherVertex(anchorPointJ)];
            neiPt = mesh_.points()[anchorPointJ];
        }
...
void Foam::hexRef8axi::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
) const
{
    ...
    forAll(cFaces, i)
    {
     ...
     if (faceMidPoint[faceI] >= 0  && faceMidPoint[faceI] != 123456789)
     {
        const face& f = mesh_.faces()[faceI];
...
void Foam::hexRef8axi::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
void Foam::hexRef8axi::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
Foam::label Foam::hexRef8axi::faceConsistentRefinement
(
    const bool maxSet,
...
void Foam::hexRef8axi::checkWantedRefinementLevels
(
...
) const
{
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        ...
        if (mag(ownLevel-neiLevel) > 1)
        {
            ...
            FatalErrorIn
            (
                "hexRef8axi::checkWantedRefinementLevels(const labelList&)"
            )   << "cell:" << own
...
void Foam::hexRef8axi::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "hexRef8axi::setInstance(const fileName& inst) : "
...
Foam::hexRef8axi::hexRef8axi(const polyMesh& mesh)
:
    mesh_(mesh),
    cellLevel_
    (
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef8axi::hexRef8axi(const polyMesh&)"
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef8axi::hexRef8axi(const polyMesh&, const labelList&"
...
Foam::labelList Foam::hexRef8axi::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef8axi::consistentRefinement : Changed " << nChanged
...
Foam::labelList Foam::hexRef8axi::consistentSlowRefinement
(
...
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRef8axi::consistentSlowRefinement"
    ...
    while (true)
    {
        if (debug)
        {
            Pout<< "hexRef8axi::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::consistentSlowRefinement : From "
...
Foam::labelList Foam::hexRef8axi::consistentSlowRefinement2
(
    const label maxFaceDiff,
    const labelList& cellsToRefine,
    const labelList& facesToCheck
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRef8axi::consistentSlowRefinement2"
            "(const label, const labelList&, const labelList&)"
        )   << "Illegal maxFaceDiff " << maxFaceDiff << nl
            << "Value should be >= 1" << exit(FatalError);
    }
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef8axi::consistentSlowRefinement2 : Changed " << nChanged
                << " refinement levels due to 2:1 conflicts."
                << endl;
        }
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::consistentSlowRefinement2 : From "
            << cellsToRefine.size() << " to " << newCellsToRefine.size()
            << " cells to refine." << endl;
...

        {
            ...
            Pout<< "hexRef8axi::consistentSlowRefinement2 : writing "
...
Foam::labelListList Foam::hexRef8axi::setRefinement
(
...
)
{
 ...
 //       const labelList& cellp = mesh_.cellPoints()[cellI];
   //    if (cellp.size() == 8)
     //  {
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef8axi::setRefinement : Dumping " << splitCells.size()
    ...
    // Find out which edges & faces can be split for non-3D cases:
    // 1. Determine type of simulation: 0 = 3D, 1 = 2D, 2 = AxiSym
    // 2. Determine face divisibility: Always for 3D, patch = wedge for 2D, patch = wedge for AxiSym
    // 3. Edges lying on divisible faces are the only divisible edges

    label simType = 0;                                                             // Default = 3D
    //if      ((mesh_.nGeometricD() == 2) && (mesh_.nSolutionD() == 2)) simType = 1; // 2D
   // else
    if ((mesh_.nGeometricD() == 2) && (mesh_.nSolutionD() == 3)) simType = 2; // AxiSym
//  else if ((mesh_.nGeometricD() == 3) && (mesh_.nSolutionD() == 3)) simType = 0; // 3D

    boolList isDivisibleFace(mesh_.nFaces(), (simType == 0) );  // true for 3D
    boolList isDivisibleEdge(mesh_.nEdges(), (simType == 0) );  // true for 3D

    if (simType == 2)  // 2D: Requires wedge face
    {
        for (label faceI = mesh_.nInternalFaces(); faceI < mesh_.nFaces(); faceI++)
        {
            const label & patchID = mesh_.boundaryMesh().whichPatch(faceI);
            if (isA<wedgePolyPatch>(mesh_.boundaryMesh()[patchID]))
            {
  ...
  //labelList cellpoints(mesh_.nCells());
 //for (label f =0; f < mesh_.nCells(); f++)
    //{
     // const labelList& pce = mesh_.cellPoints()[f];
     // Pout << "pce " << pce << endl;
    //  cellpoints[f] = mesh_.cellPoints()[f];
   // }
// Pout << "cellpointsd " << cellpoints << endl;
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        if (faceAnchorLevel[faceI] >= 0)
        {
            ...
            {
                faceMidPoint[faceI] = 123456789;    // mark to be split
            }
        }
    }
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef8axi::setRefinement : Dumping " << splitFaces.size()
          ...
          //   label cellI = pCells[pCellI];
                ...
                )
                {
                    if (nAnchorPoints[cellI] == 8)
                    {
                        ...
                        FatalErrorIn
                        (
                            "hexRef8axi::setRefinement(const labelList&"
                            ", polyTopoChange&)"
                        )   << "cell " << cellI
    ...
    //Pout << "celladedd " << cellAddedCells << endl;
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::setRefinement : Splitting faces" << endl;
    }
         ...
         if (isDivisibleFace[faceI])
           {
            ...
            forAll(f, fp)
            {
                ...
                if (pointLevel_[pointI] <= anchorLevel)
                {
                 ...
                 //  Pout << "newface1" << newFace << "owner" << own << nei << endl;
                    if (debug)
                    {
                        if (mesh_.isInternalFace(faceI))
                        {
   ...
   //             Pout << "Made it 10" << endl;
          ...
          forAll(f,fp)
           {
             ...
             if (edgeMidPoint[edgeI] >=0)
              ...
              label edgep = meshTools::findEdge(mesh_, pointI, pointJ);
    ...
    forAll(edgeMidPoint, edgeI)
    {
       ...
       //         Pout << "face from edge " << newFace << endl;
                    if (debug)
                    {
                        if (mesh_.isInternalFace(faceI))
                        {
   ...
   //     }
        ...
        {
            FatalErrorIn("hexRef8axi::setRefinement(..)")
...
void Foam::hexRef8axi::storeData
(
...
void Foam::hexRef8axi::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void Foam::hexRef8axi::updateMesh
(
...
)
{
...

    {
        ...
        {
            // Assume it is after hexRef8axi that this routine is called.
        ...
        forAllConstIter(Map<label>, cellsToRestore, iter)
        {
            ...
            {
                FatalErrorIn("hexRef8axi::updateMesh(const mapPolyMesh&)")
        ...
        //{
        //    WarningIn("hexRef8axi::updateMesh(const mapPolyMesh&)")
        //        << "Problem : "
...
)
{
    ...
    if (history_.active())
    {
        WarningIn
        (
            "hexRef8axi::subset(const labelList&, const labelList&"
            ", const labelList&)"
    ...
    // Update celllevel
    {
        ...
        if (findIndex(cellLevel_, -1) != -1)
        {
            FatalErrorIn("hexRef8axi::subset(..)")
                << "Problem : "
                << "cellLevel_ contains illegal value -1 after mapping:"
                << cellLevel_
                << abort(FatalError);
...
void Foam::hexRef8axi::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "hexRef8axi::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::hexRef8axi::checkMesh() const
{
    const scalar smallDim = 1E-6 * mesh_.bounds().mag();

    if (debug)
    {
        Pout<< "hexRef8axi::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
        ...
        forAll(patches, patchI)
        {
            ...
            if (pp.coupled())
            {
                ...
                forAll(pp, i)
                {
                    ...
                    {
                        dumpCell(own);
                        FatalErrorIn("hexRef8axi::checkMesh()")
    ...
    //  Pout << "points on faces  " << nVerts <<endl;
        // Replace data on coupled patches with their neighbour ones.
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::checkMesh : Returning" << endl;
    }
}


void Foam::hexRef8axi::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "hexRef8axi::checkRefinementLevels :"
    ...
    {
        FatalErrorIn("hexRef8axi::checkRefinementLevels(const label)")
...

    {
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (mag(cellLevel_[own] - cellLevel_[nei]) > 1)
            {
                ...
                FatalErrorIn
                (
                    "hexRef8axi::checkRefinementLevels(const label)"
...
Foam::labelList Foam::hexRef8axi::getSplitPoints(const label axis, const scalar axisVal) const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::getSplitPoints :"
    ...
    if (!history_.active())
    {
        FatalErrorIn("hexRef8axi::getSplitPoints()")
    ...
    for (label pointI = 0; pointI < mesh_.nPoints(); pointI++)
    {
        ...
        if (pCells.size() != 4 || coord[axis] >= axisVal)
        {
    ...
    //    Pout << "visiblecells " << visibleCells[cellI] << endl;
    //    Pout << "parent index " << history_.parentIndex(cellI) << endl;
...
//void Foam::hexRef8axi::markIndex
//(
...
//void Foam::hexRef8axi::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
Foam::labelList Foam::hexRef8axi::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "hexRef8axi::consistentUnrefinement :"
    ...
    if (maxSet)
    {
        FatalErrorIn
        (
            "hexRef8axi::consistentUnrefinement(const labelList&, const bool"
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (ownLevel < (neiLevel-1))
            {
                ...
                else
                {
                    ...
                    // {
                    //     FatalErrorIn("hexRef8axi::consistentUnrefinement(..)")
...
void Foam::hexRef8axi::setUnrefinement
(
...
)
{
    if (!history_.active())
    {
        FatalErrorIn
        (
            "hexRef8axi::setUnrefinement(const labelList&, polyTopoChange&)"
        )   << "Only call if constructed with history capability"
            << abort(FatalError);
    }
    ...
    if (debug)
    {
        Pout<< "hexRef8axi::setUnrefinement :"
            << " Checking initial mesh just to make sure" << endl;
        ...
        Pout<< "hexRef8axi::setRefinement : Dumping " << pSet.size()
            << " points and "
    ...
    {
        labelHashSet splitFaces(4*splitPointLabels.size());
    ...
    //  Pout<<"face to split"<<splitFaces<<endl;
  ...
  //   Pout << "faces to remove " << mesh_.faces()[facesToRemove[0]] << endl;
  //  Pout << "faces to remove " << mesh_.edgeFaces()[8817] << endl;
  //  Pout << "faces to remove " << mesh_.faces()[facesToRemove[2]] << endl;
  //  Pout << "faces to remove " << mesh_.faces()[facesToRemove[3]] << endl;
    ...
    forAll(splitPointLabels, i)
    {
        ...
        //if (debug)
        {
            ...
            forAll(pCells, j)
            {
                ...
                if (region == -1)
                {
                    FatalErrorIn("hexRef8axi::setUnrefinement(..)")
                        << "Ininitial set of split points to unrefine does not"
...
bool Foam::hexRef8axi::write() const
{
