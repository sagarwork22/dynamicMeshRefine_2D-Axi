...
namespace Foam
{
    defineTypeNameAndDebug(hexRef82D, 0);
...
)
{
    ...
    forAll(elems, i)
    {
        ...
        if (newI >= len)
        {
            FatalErrorIn("hexRef82D::reorder(..)") << abort(FatalError);
        }
...
void Foam::hexRef82D::getFaceInfo
(
    const label faceI,
    label& patchID,
    label& zoneID,
...
Foam::label Foam::hexRef82D::addFace
(
...
// for the face)
Foam::label Foam::hexRef82D::addInternalFace
(
    polyTopoChange& meshMod,
...
Foam::scalar Foam::hexRef82D::getLevel0EdgeLength() const
{
    if (cellLevel_.size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef82D::getLevel0EdgeLength() const"
    ...
    if (debug)
    {
        Pout<< "hexRef82D::getLevel0EdgeLength() :"
    ...
    if (level0Size == -1)
    {
        FatalErrorIn("hexRef82D::getLevel0EdgeLength()")
...
void Foam::hexRef82D::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label faceI,
...
Foam::label Foam::hexRef82D::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef82D::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef82D::countAnchors
(
    const labelList& f,
...
void Foam::hexRef82D::dumpCell(const label cellI) const
{
    OFstream str(mesh_.time().path()/"cell_" + Foam::name(cellI) + ".obj");
    Pout<< "hexRef82D : Dumping cell as obj to " << str.name() << endl;
...
Foam::label Foam::hexRef82D::findLevel
(
    const label faceI,
    const face& f,
    const label startFp,
...
) const
{
    ...
    forAll(f, i)
    {
        ...
        if (pointLevel_[pointI] < wantedLevel)
        {
            ...
            FatalErrorIn("hexRef82D::findLevel(..)")
                << "face:" << f
...
Foam::label Foam::hexRef82D::getAnchorLevel(const label faceI) const
{
    const face& f = mesh_.faces()[faceI];
...
void Foam::hexRef82D::checkInternalOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
void Foam::hexRef82D::checkBoundaryOrientation
(
    polyTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
// splitPoint.
void Foam::hexRef82D::insertEdgeSplit
(
...
Foam::label Foam::hexRef82D::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
       ...
       forAll(cFaces, i)
         ...
         //     const label& patchID = mesh_.boundaryMesh().whichPatch(faceJ);
             if (cellMidPoint[faceJ] != faceMidPointI
                ...
                && cellMidPoint[faceJ] != 123456789 )//isA<emptyPolyPatch>(mesh_.boundaryMesh()[patchID]) )
               {
                  face1  = faceJ;
...
void Foam::hexRef82D::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
void Foam::hexRef82D::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
void Foam::hexRef82D::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
Foam::label Foam::hexRef82D::faceConsistentRefinement
(
    const bool maxSet,
...
void Foam::hexRef82D::checkWantedRefinementLevels
(
...
) const
{
    ...
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
    {
        ...
        if (mag(ownLevel-neiLevel) > 1)
        {
            ...
            FatalErrorIn
            (
                "hexRef82D::checkWantedRefinementLevels(const labelList&)"
            )   << "cell:" << own
...
void Foam::hexRef82D::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "hexRef82D::setInstance(const fileName& inst) : "
...
Foam::hexRef82D::hexRef82D(const polyMesh& mesh)
:
    mesh_(mesh),
    cellLevel_
    (
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef82D::hexRef82D(const polyMesh&)"
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "hexRef82D::hexRef82D(const polyMesh&, const labelList&"
...
Foam::labelList Foam::hexRef82D::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef82D::consistentRefinement : Changed " << nChanged
...
Foam::labelList Foam::hexRef82D::consistentSlowRefinement
(
...
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRef82D::consistentSlowRefinement"
    ...
    while (true)
    {
        if (debug)
        {
            Pout<< "hexRef82D::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        Pout<< "hexRef82D::consistentSlowRefinement : From "
...
Foam::labelList Foam::hexRef82D::consistentSlowRefinement2
(
    const label maxFaceDiff,
    const labelList& cellsToRefine,
    const labelList& facesToCheck
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "hexRef82D::consistentSlowRefinement2"
            "(const label, const labelList&, const labelList&)"
        )   << "Illegal maxFaceDiff " << maxFaceDiff << nl
            << "Value should be >= 1" << exit(FatalError);
    }
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef82D::consistentSlowRefinement2 : Changed " << nChanged
                << " refinement levels due to 2:1 conflicts."
                << endl;
        }
    ...
    if (debug)
    {
        Pout<< "hexRef82D::consistentSlowRefinement2 : From "
            << cellsToRefine.size() << " to " << newCellsToRefine.size()
            << " cells to refine." << endl;
...

        {
            ...
            Pout<< "hexRef82D::consistentSlowRefinement2 : writing "
...
Foam::labelListList Foam::hexRef82D::setRefinement
(
 ...
 //       const labelList& cellp = mesh_.cellPoints()[cellI];
   //    if (cellp.size() == 8)
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef82D::setRefinement : Dumping " << splitCells.size()
    ...
    // 2. Determine face divisibility: Always for 3D, patch = empty for 2D, patch = wedge for AxiSym
    ...
    else if ((mesh_.nGeometricD() == 2) && (mesh_.nSolutionD() == 3)) simType = 2; // AxiSym
//  else if ((mesh_.nGeometricD() == 3) && (mesh_.nSolutionD() == 3)) simType = 0; // 3D
    ...
    if (simType == 1)  // 2D: Requires empty face
    {
        for (label faceI = mesh_.nInternalFaces(); faceI < mesh_.nFaces(); faceI++)
        {
    ...
    else// if (simType == 0)  // AxiSym: Requires wedge face
    {
        for (label faceI = mesh_.nInternalFaces(); faceI < mesh_.nFaces(); faceI++)
        {
            const label & patchID = mesh_.boundaryMesh().whichPatch(faceI);
            if (isA<symmetryPolyPatch>(mesh_.boundaryMesh()[patchID]))
            {
                isDivisibleFace[faceI] = true;
                const labelList& fEdges = mesh_.faceEdges(faceI);
                forAll(fEdges, i)
...
// Pout << "cellpointsd " << cellpoints << endl;
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef82D::setRefinement : Dumping " << splitFaces.size()
    ...
    //       Pout << "anchor" << cellAnchorPoints << endl;
        forAll(cellMidPoint, cellI)
        {
            if (cellMidPoint[cellI] >= 0)
            {
    ...
    if (debug)
    {
        Pout<< "hexRef82D::setRefinement : Splitting faces" << endl;
    }
  ...
  //          Pout << "nei" << nei <<  endl;
   ...
   //     }
        ...
        {
            FatalErrorIn("hexRef82D::setRefinement(..)")
...
void Foam::hexRef82D::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void Foam::hexRef82D::updateMesh
(
...
)
{
...

    {
        ...
        {
            // Assume it is after hexRef82D that this routine is called.
        ...
        forAllConstIter(Map<label>, cellsToRestore, iter)
        {
            ...
            {
                FatalErrorIn("hexRef82D::updateMesh(const mapPolyMesh&)")
        ...
        //{
        //    WarningIn("hexRef82D::updateMesh(const mapPolyMesh&)")
        //        << "Problem : "
...
)
{
    ...
    if (history_.active())
    {
        WarningIn
        (
            "hexRef82D::subset(const labelList&, const labelList&"
            ", const labelList&)"
...
void Foam::hexRef82D::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "hexRef82D::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::hexRef82D::checkMesh() const
{
    const scalar smallDim = 1E-6 * mesh_.bounds().mag();

    if (debug)
    {
        Pout<< "hexRef82D::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
        ...
        forAll(patches, patchI)
        {
            ...
            if (pp.coupled())
            {
                ...
                forAll(pp, i)
                {
                    ...
                    {
                        dumpCell(own);
                        FatalErrorIn("hexRef82D::checkMesh()")
    ...
    if (debug)
    {
        Pout<< "hexRef82D::checkMesh : Returning" << endl;
    }
}


void Foam::hexRef82D::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "hexRef82D::checkRefinementLevels :"
    ...
    {
        FatalErrorIn("hexRef82D::checkRefinementLevels(const label)")
...

    {
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (mag(cellLevel_[own] - cellLevel_[nei]) > 1)
            {
                ...
                FatalErrorIn
                (
                    "hexRef82D::checkRefinementLevels(const label)"
...
Foam::labelList Foam::hexRef82D::getSplitPoints(const label axis, const scalar axisVal) const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
    ...
    if (!history_.active())
    {
        FatalErrorIn("hexRef82D::getSplitPoints()")
...
//void Foam::hexRef82D::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
Foam::labelList Foam::hexRef82D::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "hexRef82D::consistentUnrefinement :"
    ...
    if (maxSet)
    {
        FatalErrorIn
        (
            "hexRef82D::consistentUnrefinement(const labelList&, const bool"
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (ownLevel < (neiLevel-1))
            {
                ...
                else
                {
                    ...
                    // {
                    //     FatalErrorIn("hexRef82D::consistentUnrefinement(..)")
...
void Foam::hexRef82D::setUnrefinement
(
...
)
{
    if (!history_.active())
    {
        FatalErrorIn
        (
            "hexRef82D::setUnrefinement(const labelList&, polyTopoChange&)"
        )   << "Only call if constructed with history capability"
            << abort(FatalError);
    }
    ...
    if (debug)
    {
        Pout<< "hexRef82D::setUnrefinement :"
            << " Checking initial mesh just to make sure" << endl;
        ...
        Pout<< "hexRef82D::setRefinement : Dumping " << pSet.size()
            << " points and "
 ...
 //    Pout << "faces to remove " << facesToRemove << endl;
    ...
    forAll(splitPointLabels, i)
    {
        ...
        //if (debug)
        {
            ...
            forAll(pCells, j)
            {
                ...
                if (region == -1)
                {
                    FatalErrorIn("hexRef82D::setUnrefinement(..)")
                        << "Ininitial set of split points to unrefine does not"
...
bool Foam::hexRef82D::write() const
{
