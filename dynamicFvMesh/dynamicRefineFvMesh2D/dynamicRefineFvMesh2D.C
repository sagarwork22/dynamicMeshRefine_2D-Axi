...
)
{
    ...
    if (debug)
    {
        ...
        for (label faceI = 0; faceI < nInternalFaces(); faceI++)
        {
            ...
            {
                FatalErrorIn("dynamicRefineFvMesh2D::refine(const labelList&)")
    ...
    */
        ...
        forAll(faceMap, faceI)
        {
            ...
            if (oldFaceI >= 0)
            {
                ...
                if (masterFaceI < 0)
                {
                    FatalErrorIn
                    (
                        "dynamicRefineFvMesh2D::refine(const labelList&)"
	...
	//-----REDACTED----//
        forAllConstIter(HashTable<surfaceScalarField*>, fluxes, iter)
        {
            ...
            {
                WarningIn("dynamicRefineFvMesh2D::refine(const labelList&)")
...
Foam::scalarField
Foam::dynamicRefineFvMesh2D::minCellField(const volScalarField& vFld) const
{
...
Foam::labelList Foam::dynamicRefineFvMesh2D::selectUnrefinePoints
(
    const label axis,
...
bool Foam::dynamicRefineFvMesh2D::update()
{
    ...
    {
        FatalErrorIn("dynamicRefineFvMesh2D::update()")
...
) const
{
    ...
    const_cast<hexRef82D&>(meshCutter_).setInstance(time().timeName());
